...
const StateTab = ({ name, isActive, onClick }) => (
  <button onClick={onClick}
    className={`pb-2 text-lg font-medium transition-colors ${isActive ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-800' }`}
  >
    {name}
  </button>
);
...
const StatBox = ({ value, label }) => (
  <div className="flex-1 text-center p-4 rounded-lg border-2" style={{ borderColor: '#2563EB' }} >
...
// ==========================================
// 2. PAGE SECTIONS (Hardcoded Content)
// ==========================================

const Header = ({ onBack }) => {
  const [activesearch, setActivesearch] = useState("All");
  const searchTabs = ["All", "Industry", "Sectors", "Name"];

  return (
    <div className="text-left text-white bg-blue-800 p-8">
      {onBack && (
        <button onClick={onBack} className="mb-6 px-4 py-2  text-white rounded-lg hover:tranform hover:scale-150 transition-transform">
          ←
        </button>
      )}
      ...
      <p className="text-[16px] max-w-3xl px-4   text-blue-100">
        Company data includes essential details about businesses like name, location and industry. Here you’ll find a guide and our selection of top company data sources.
      </p>
      <div className="mt-6 gap-8  flex px-4">
        <input type="text" className="pl-2 pr-4 bg-white text-sm h-[40px]  min-w-[400px] text-gray-600   rounded-lg focus:outline-none focus:ring-2 focus:ring-[#003366]" />
      ...
      <div className="flex flex-wrap px-4">
        {searchTabs.map((tab) => (
          <button key={tab} onClick={() => setActivesearch(tab)}
            className={`mt-2 mr-4 px-2 py-1  rounded-lg text-sm  hover:bg-white hover:text-black ${activesearch === tab ? 'bg-white text-black' : 'text-white'}`}>
            {tab}
          </button>
        ))}
      </div>
...
const SchemesGrid = () => {
  ...
  const schemes = [
    ...
    // ... Duplicated entries for layout demo ...
    {
        country: "India", status: "Actively raising", logoUrl: "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c1/State_emblem_of_India_%28Ashoka_Chakra%29.svg/1200px-State_emblem_of_India_%28Ashoka_Chakra%29.svg.png",
        title: "Agri Loan-kisan", type: "(Credit card)", isVerified: true, location: "Delhi", ministry: "Ministry of Agriculture",
        description: "Aims to provide farmers with timely and adequate credit...",
    ...
    <div className="bg-blue-800 text-white mt-16 p-20 rounded-xl flex flex-col md:flex-row items-center gap-8">
      <div className="flex-1   ">
        ...
        <p className="mt-8 text-[16px] max-w-[65%] font-[inter]  font-500 font-medium text-blue-100">
          We offer the most comprehensive and reliable database solutions to empower businesses across industries...
        </p>
      </div>
      <img src={phone} alt="mobile preview" className="w-[440px]  h-[360px] object-cover rounded-lg shadow-lg" />
    </div>
  );
};
  ...
  return (
    <div className="mt-16">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 flex flex-wrap  ">
        {industries.map((t, i) => (
          <span key={i} className="px-4 py-2 bg-gray-50 text-center border border-gray-200 rounded-full text-[20px] font-[inter] cursor-pointer hover:bg-blue-100 hover:border-blue-200">
            {t}
          </span>
        ))}
      </div>
...
const SchemesInStates = () => {
  ...
  const sidebarQuestions = [ 'What is a category franchise?', 'Who is the target customer?', 'What is the business model?', 'Services offered?', 'Infrastructure required?', 'Investment and ROI?', 'Training provided?', 'Best locations?' ];
  const products = [
    { icon: Success, title: 'Success.ai', description: 'Success.ai | B2B Company Data...', location: 'UK, USA, Germany' },
    { icon: Salutary, title: 'Salutary Data', description: 'Company Data | 6.7M+ Companies...', location: 'Lexington, USA' },
    { icon: Core, title: 'CoreSignal', description: 'Coresignal | Firmographic Data...', location: 'UK, USA, Germany' }
  ];
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="flex justify-between items-center flex-wrap gap-4 border-b border-gray-200">
        <div className="flex gap-6 flex-wrap">
          {states.map(state => (
            <StateTab key={state} name={state} isActive={activeState === state} onClick={() => setActiveState(state)} />
          ))}
        </div>
        <div className="relative">
          <input type="text" placeholder="Search States" className="pl-10 pr-4 py-2 bg-gray-100 rounded-full focus:outline-none" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-8">
        <div className="lg:col-span-2 flex flex-col md:flex-row gap-6">
          <img src={AndhraPradesh} alt="Map of Andhra Pradesh" className="w-full md:w-1/3 h-auto object-contain rounded-lg" />
          <div className="flex-1">
        ...
        <div className="bg-gray-100 rounded-lg p-6 h-fit">
          ...
          <div className="space-y-3">
            {sidebarQuestions.map((q, i) => (
              <a href="#" key={i} className="block text-blue-600 text-sm hover:underline">{q}</a>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-12">
        {products.map(product => (
          <ProductCard key={product.title} icon={product.icon} title={product.title} description={product.description} location={product.location} />
        ))}
      </div>
    </div>
  );
...
export default function SchemeListings({ onBack }) {
  ...
  const [flags, setFlags] = useState({
    showHeader: true,
    showSchemesGrid: true,
    showLeadingProvider: true,
    showIndustryTags: true,
    showSchemesInStates: true
  });

  // 2. FETCH: Get visibility control from Backend
  useEffect(() => {
    axios.get("http://localhost:5000/api/scheme-listings/flags")
      .then((res) => {
        if (res.data) {
          setFlags(res.data);
        }
  ...
  // 3. RENDER: Conditionally render components based on flags
  return (
    <div className="bg-white">
      {flags.showHeader && <Header onBack={onBack} />}
      
      <div className="px-20 py-10">
        {flags.showSchemesGrid && <SchemesGrid />}
        {flags.showLeadingProvider && <LeadingProvider />}
        {flags.showIndustryTags && <IndustryTags />}
        {flags.showSchemesInStates && <SchemesInStates />}
      </div>
    </div>
  );
}