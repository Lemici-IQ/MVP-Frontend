...
import { Settings, CalendarCheck, User } from 'lucide-react';
...
// --- SECTIONS (Hardcoded Content) ---

const Hero = () => (
    <section className="px-6 py-40 md:px-12 lg:px-20">
        <div className="text-center mb-50">
            <h1 className="text-4xl md:text-5xl font-bold text-purple-900 mb-4"
            style={{ fontFamily: "Space Grotesk, sans-serif", fontWeight: 800, color: "#652C90" }}>
                Business Solution
            </h1>
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12 lg:gap-20">
            ...
            <div className="lg:w-1/2 w-full">
                <div className="relative rounded-3xl overflow-hidden shadow-2xl bg-black">
                    <img src={a} alt="Modern dark workspace setup" className="w-full h-[400px] object-cover opacity-80"/>
                </div>
            </div>
        </div>
    </section>
...
const IndustrySolutions = () => (
    <section className="px-6 py-16 md:px-12 lg:px-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <IndustryCard image={e} title="Agriculture" desc="Helps food companies, governments, investors, and agriculture players make better pricing, crop selection, and sales and marketing decisions" />
            <IndustryCard image={f} title="Financial Service" desc="Standardized market-sizing database that covers payments products data globally." />
            <IndustryCard image={g} title="Automotive" desc="The LeMiCi Center for Future Mobility brings deep expertise and leading insights to help clients navigate an autonomous, connected, electrified, and shared future." />
            <IndustryCard image={h} title="Healthcare" desc="We empower leaders across the healthcare value chain with data, analytics, and intelligence solutions to improve healthcare." />
            <IndustryCard image={i} title="Aerospace" desc="Helps leaders in the commercial defense sector make fast, informed strategic decisions using the world's most comprehensive U.S. Department of Defense database" />
            <IndustryCard image={j} title="Electrical Power" desc="Improve network reliability and reduce costs with predictive asset maintenance" />
            <IndustryCard image={k} title="Natural Gas" desc="Helps energy companies manage uncertainty and improve performance through analysis, insights, and benchmarking" />
            <IndustryCard image={l} title="Metal & Mining" desc="Provides mine operators strategic business intelligence needed to identify, quantify, and prioritize value creation opportunities" />
        </div>
    </section>
);
...
const FunctionSolutions = () => (
    <section className="px-6 py-16 md:px-12 lg:px-20 ">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <IndustryCard image={m} title="Operations" desc="Helps food companies, governments, investors, and agriculture players make better pricing, crop selection, and sales and marketing decisions" />
            <IndustryCard image={n} title="Sustainability" desc="Accelerating decarbonization transformations at scale" />
            <IndustryCard image={o} title="Marketing & Sales" desc="Improves return on sales through better pricing, promotions, assortment, and performance management" />
            <IndustryCard image={p} title="Organisation" desc="Applies data and rigor to the most important organizational decisions." />
            <IndustryCard image={q} title="Digital" desc="Watchtower helps leaders and teams proactively identify and address the greatest risks in their technology-project portfolios by leveraging best-in-class analytics." />
            <IndustryCard image={r} title="Transform" desc="Tracks initiatives, milestones, and impact in transformations through a program management platform." />
            <IndustryCard image={s} title="Risk" desc="Advanced solutions for customer assistance and credit loss mitigation" />
            <IndustryCard image={t} title="Strategy & Corporate Finance" desc="Provides fast, sophisticated financial insights through automated benchmarking reports" />
        </div>
    </section>
);
    ...
    <section className="px-6  md:px-12 lg:px-20">
        <div className="bg-purple-800 rounded-3xl p-8 md:p-12 lg:p-16 flex flex-col-[50%_50%] md:flex-row items-center  overflow-hidden relative">
            ...
            <div className="md:w-1/2 flex justify-center md:justify-end z-10 w-full">
                <img src={u} alt="Career Illustration" className="  object-contain drop-shadow-2xl transform -md:translate-x-25 translate-y-16" />
            </div>
...
// --- MAIN COMPONENT ---
const Solution = () => {
  // 1. STATE: Initialize flags with safe defaults (all true)
  const [flags, setFlags] = useState({
    showHero: true,
    showAssessment: true,
    showIndustrySolutions: true,
    showFunctionSolutions: true,
    showCareerCTA: true
  });

  // 2. FETCH: Get flags from backend on mount
  useEffect(() => {
    axios.get("http://localhost:5000/api/solution/flags")
      .then((res) => {
        if (res.data) {
          setFlags(res.data);
        }
      ...
      .catch((err) => {
        console.error("Error fetching solution flags:", err);
      });
  }, []);

  // 3. RENDER: Conditional rendering based on flags
  return (
    <div className="min-h-screen bg-white font-sans text-gray-800">
      <div className="max-w-[1440px] mx-auto w-full bg-white overflow-hidden">
        ...
        {flags.showAssessment && <AssessmentTool />}
        
        {flags.showIndustrySolutions && <IndustrySolutions />}
        
        {flags.showFunctionSolutions && <FunctionSolutions />}
        
        {flags.showCareerCTA && <CareerCTA />}
