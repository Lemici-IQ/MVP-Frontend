  ...
  return (
    <div className="bg-white w-full min-h-screen flex flex-col items-center py-12 sm:py-16 lg:py-24 px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center mb-8 sm:mb-10 lg:mb-12 space-y-4 max-w-4xl">
        ...
        <div className="flex flex-col items-center">
          ...
          <p className="mt-2 text-xs sm:text-sm font-semibold text-blue-600 text-center px-4">
            {billing === 'Annual' ? '-15% off on annual payments. Try LeMiVerse for 14 days.' : '\u00A0'}
          </p>
        </div>
      </div>
      ...
      <div className="w-full">
        {/* Desktop/Tablet View */}
        <div className="hidden sm:flex sm:justify-center sm:space-x-4 lg:space-x-6 sm:overflow-x-auto sm:px-4 lg:px-8 pb-12 pt-8">
          {plans.map((p, idx) => (
            <div key={idx} className="relative flex-shrink-0">
              {(p.isPopular || selectedPlan === idx) && (
                <div className="absolute top-0 left-[50%] -translate-x-1/2 bg-orange-500 px-4 lg:px-6 py-1 lg:py-2 rounded-full text-[10px] lg:text-[12px] font-semibold text-white z-10">
                  MOST POPULAR
                </div>
              )}
              <div
                ...
                className={`w-[200px] sm:w-[220px] lg:w-[250px] bg-white cursor-pointer transition-all duration-200 mt-6 rounded-[20px] lg:rounded-[24px] ${
                  selectedPlan === idx
                    ? 'border-6 lg:border-8 border-orange-500 shadow-orange-200 shadow-lg'
                    : p.isPopular
                    ? 'border-4 lg:border-6 border-orange-500'
                    : 'border border-gray-200'
                } shadow-md hover:shadow-lg`}
              >
                <div className="p-4 sm:p-6 lg:p-8 flex flex-col space-y-3 lg:space-y-4">
                  <div>
                  ...
                  <button className={`mt-3 lg:mt-4 w-full py-2.5 lg:py-3 rounded-full text-xs lg:text-sm font-medium transition-colors ${
                    selectedPlan === idx 
        ...
        <div className="sm:hidden space-y-6 px-4 pb-8 pt-4">
          {plans.map((p, idx) => (
            <div key={idx} className="relative">
              {(p.isPopular || selectedPlan === idx) && (
                <div className="absolute top-[-8px] left-[50%] -translate-x-1/2 bg-orange-500 px-4 py-1 rounded-full text-[10px] font-semibold text-white z-10">
                  MOST POPULAR
                </div>
              )}
              <div
              ...
              >
                <div className="p-6 flex flex-col space-y-4">
                  ...
                  <button className={`w-full py-3 rounded-full text-sm font-medium transition-colors ${
                    selectedPlan === idx 
                      ? 'bg-orange-500 text-white hover:bg-orange-600' 
                      : 'bg-black text-white hover:bg-gray-800'
                  }`}>
      ...
      <div className="w-full max-w-4xl px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-12">
        <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
          ...
          <div>
            <p className="font-inter font-medium text-lg sm:text-xl">Education &gt;</p>
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-3 sm:space-y-0 sm:space-x-4">
          <div className="w-10 h-10 sm:w-12 sm:h-12 bg-black rounded" />
          <div>
            <p className="font-inter font-medium text-lg sm:text-xl">Not for‑profit &gt;</p>
            <p className="text-sm text-gray-600">
