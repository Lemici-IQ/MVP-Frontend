...
const NewChatbot = () => {
  ...
  const resolveRoute = useCallback(
    (text) => {
      ...
      if (/invest(or|ment)|fund/i.test(q))
        return "/abhinay/startups-zone-investorhub";
      ...
      if (/schemes?|subsid(y|ies)|grant|msme|government|govt/i.test(q))
        return "/abhinay/offerings/govtnavigator";
      if (/product|catalog|list/i.test(q)) return "/abhinay/products";
      if (/compliance|license|registration/i.test(q))
        return "/abhinay/compliance";
      if (/partner|collab/i.test(q)) return "/abhinay/partnership";
      if (/train|skill|growth/i.test(q)) return "/abhinay/training";
      if (/account|login|signup|sign up|register/i.test(q))
        return "/abhinay/account";
  ...
  const handleSubmit = useCallback(() => {
    ...
    if (route) {
      ...
      toast(
        "I couldn't find a matching page. Try keywords like 'franchise', 'investment', 'govt schemes', 'products', 'offers', 'compliance', 'partnership', 'training', 'account', or 'support'."
      );
    }
